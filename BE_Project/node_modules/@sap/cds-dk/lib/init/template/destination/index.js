...
module.exports = class DestinationTemplate extends require('../../plugin') {
    ...
    async combine(facet) {
        ...
        const { addApprouter, hasApprouter, addMultitenancy, hasMultitenancy, addKyma, addMta, hasMta, srv, isJava } = project

        if (addMta || (addApprouter || addMultitenancy) && hasMta) {
            const srvModule = srv4(srv.path)
            const additions = [srvModule, destination]
            const relationships = [{
        ...
        if (addKyma) {
            ...
            await KymaTemplate.mergeDependency('service-instance', 'destination')
        }
    }
}
