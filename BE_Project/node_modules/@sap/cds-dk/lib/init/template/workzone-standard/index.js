...
const { destination, destinations, xsuaa, srv4, html5RepoHost, mtxSidecar4 } = require('../../registries/mta')
const { srvApi } = require('../../registries/xs-app')
...
module.exports = class extends require('../../plugin') {
  ...
  async combine(facet) {
    ...
    const { isJava, addMta, hasMta, srv, apps, appPath, appUIPaths, addMultitenancy, addXsuaa, strippedAppName } = project
    ...
    await Promise.all(apps.map(async ({ app }) => {
      const additions = [srvApi]
      const overwrites = [{
        item: [srvApi, 'destination'],
        withValue: `srv-api`
      }]
      project.app = app
    ...
    if (addMta || (addMultitenancy || addXsuaa) && hasMta) {
      ...
      const additions = [srvModule, destinations, destination, {
      ...
      if (addXsuaa) {
        relationships.push({
          insert: [xsuaa, 'name'],
          into: [destinations, 'requires']
        })
        additions.push({
            in: [destinations, 'parameters.content.instance.destinations' ],
            where: { Authentication: 'OAuth2UserTokenExchange' }
        })
      }
