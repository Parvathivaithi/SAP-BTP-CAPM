...
module.exports = class extends require('../../plugin') {
  ...
  async run() {
    ...
    if (isNodejs) {
      ...
      await mvn.add('APPLICATION_LOGGING')
    }
  }

  async combine(facet) {
    const project = readProject(facet)
    const { hasMta, addMta, addKyma, addApprouter, hasApprouter, isJava, addMultitenancy, hasMultitenancy, srv } = project
    ...
    if (addKyma) {
      ...
      await KymaTemplate.mergeDependency('service-instance', 'cloud-logging')
    }
  }
}
