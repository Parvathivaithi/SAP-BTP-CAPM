...
const { ODATA_VERSION_V2, FOLDER_GEN, CONTENT_EDMX, CONTENT_PACKAGELOCK_JSON,
    CONTENT_NPMRC, CONTENT_CDSRC, CONTENT_ENV, CONTENT_DEFAULT_ENV_JSON, FLAVOR_LOCALIZED_EDMX,
    OPTION_WS_PACK, SEVERITY_WARNING } = require('../../constants');
const LOG = cds.log('cli|build')
const DEBUG = cds.debug('cli|build')
const NODE_ENGINE_VERSION = '^24'
const { exec } = require('node:child_process')
const pexec = require('node:util').promisify(exec)
const escapeLineBreaks = value => value.replace(/\r/g, '\\u000D').replace(/\n/g, '\\u000A')
...
module.exports = class NodejsBuildPlugin extends require('../edmx') {
    ...
    async build() {
        ...
        if (this.isStagingBuild()) {
            ...
            await this.#copyWorkspaceLock(destRoot)
            try {
    ...
    // copy the workspace root lock into gen/srv when the module has none, so monorepo deployments install frozen dependencies
    async #copyWorkspaceLock(destRoot) {
        if (this.hasBuildOption(CONTENT_PACKAGELOCK_JSON, false)) return
        if (exists(path.join(destRoot, 'package-lock.json'))) return
        const wsRoot = findWorkspaceRoot(cds.root)
        if (!wsRoot || wsRoot === cds.root) return
        const rootLock = path.join(wsRoot, 'package-lock.json')
        if (!exists(rootLock)) return
        DEBUG?.(`Copying workspace root package-lock.json from '${wsRoot}' into build destination`)
        await this.copy(rootLock).to(path.join(destRoot, 'package-lock.json'))
    }
    ...
    async copySrvContent(src, destRoot, destSrv) {
        const srvRootBlockList = RegExp('package\\.json$|package-lock\\.json$|\\.npmrc$|\\.cdsrc\\.json$|\\.cdsrc\\.yaml$')
        ...
        await super.copyNativeContent(src, destSrv, (entry) => {
            ...
            // skip messages bundles written by collectAllLanguageBundles; copying source files
            // afterwards would overwrite them with the (potentially empty) project originals
            if (/(\/|\\)_i18n(\/|\\)messages(_\w+)?\.properties$/.test(entry)) return false
        ...
        srvAllowList += !this.hasBuildOption(CONTENT_CDSRC, false) ? "|\\.cdsrc\\.json$|\\.cdsrc\\.yaml$" : ""
    ...
    async _packWorkspaceDependencies(dest) {
        ...
        await this._packWorkspaceRoot(dest, wsRoot, appPkg)
    }

    async _packWorkspaceRoot(dest, wsRoot, appPkg) {
        if (!wsRoot || wsRoot === cds.root) return
        // The workspace root package is not a member of its own `workspaces` globs, so
        // `getWorkspaces` never returns it. Pack it explicitly when the app depends on it
        // via a `file:` spec that resolves to the workspace root.
        const rootName = require(path.join(wsRoot, 'package.json')).name
        const spec = appPkg.dependencies?.[rootName]
        if (!spec?.startsWith('file:')) return
        if (path.resolve(cds.root, spec.slice('file:'.length)) !== path.resolve(wsRoot)) return
        let pkgDescriptors
        try {
            pkgDescriptors = await NodejsBuildPlugin._execNpmPack(dest, wsRoot)
        } catch (e) {
            DEBUG?.(e)
            throw new BuildError(`Failed to package npm workspace root\n${e.message}`)
        }
        const pkgDescriptor = pkgDescriptors?.find(d => d.name === rootName)
        if (!pkgDescriptor) return
        const { filename: fileName } = pkgDescriptor
        appPkg.dependencies[rootName] = `file:${fileName}`
        this.pushFile(path.join(dest, fileName))
        await this.write(JSON.stringify(appPkg, 0, 2)).to(path.join(dest, 'package.json'))
    }
     ...
     * @param {string[]} [workspaces] - list of workspace names; when omitted, the workspace root package itself is packed
     * @returns {Promise<object[]>} - array of pack descriptors ({ name, filename, ... })
     * @private
     */
    static async _execNpmPack(dest, wsRoot, workspaces) {
        ...
        // `npm pack --json` returns an array up to npm 11, but an object keyed by
        // package name as of npm 12 — normalize to an array of descriptors.
        return Array.isArray(parsed) ? parsed : Object.values(parsed)
    }

    async collectLanguageBundles(model, bundleDest) {
        const result = await super.collectLanguageBundles(model, bundleDest)
        if (result) await this.collectMessageBundles(bundleDest, model)
        return result
    }

    async collectMessageBundles(bundleDest, model) {
        const messageBundle = cds.i18n.bundle4('messages', { model })
        ...
        for (const [locale, messages] of Object.entries(messagesData)) {
            if (Object.keys(messages).length > 0) {
                ...
                const content = Object.entries(messages).map(([k, v]) => `${k}=${escapeLineBreaks(v)}`).join('\n') + '\n'
                await this.write(content).to(path.join(bundleDest, fileName))
            }
        }
    }
}
