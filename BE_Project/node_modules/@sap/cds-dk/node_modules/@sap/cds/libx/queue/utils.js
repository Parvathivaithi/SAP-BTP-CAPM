...
const create_task = (queueName, msg, context, serviceName, queueOpts, reoccurrence) => {
  ...
  const taskMsg = {
    ...
    timestamp,
    // reset status so re-submitting a task that is currently being processed (UPSERT over its
    // 'processing' row) yields a processable row again instead of a stuck one
    status: null,
