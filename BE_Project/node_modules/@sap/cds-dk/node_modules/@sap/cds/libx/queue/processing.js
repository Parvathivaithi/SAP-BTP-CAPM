...
const _timestampPlusOneMS = ts => new Date(ts.getTime() + 1).toISOString()

let _processableTasksQuery = {}

const _inlineLiterals = w => {
  if (w.xpr) return w.xpr.map(xpr => _inlineLiterals(xpr))
  // also recurse into function args (e.g. `startswith(target, '<appid>')`) so a literal arg isn't
  // bound as a `?` and shift the positional binding of the query's real params
  if (w.args) return (Array.isArray(w.args) ? w.args : Object.values(w.args)).map(a => _inlineLiterals(a))
...
const _getProcessableTasksQuery = ({ maxAttempts, chunkSize }, queueName) => {
  ...
  // also match pre-`appid` rows: `appid IS NULL` with `target` starting with `appid` (the former app prefix)
  const targetCondition = appid
    ? `(appid = '${appid}' AND target = '${queueName}') OR (appid IS NULL AND startswith(target, '${appid}') = true)`
    : `target = '${queueName}'`
  ...
  _processableTasksQuery[queueName].SELECT.where.map(w => _inlineLiterals(w))
  _processableTasksQuery[queueName].SELECT.columns = Object.keys(entity.elements).map(k => ({ ref: [k] }))
  _processableTasksQuery[queueName].forUpdate({ ignoreLocked: true })
...
const _getFlushTasksQuery = ({ maxAttempts, chunkSize }, queueName) => {
  if (_flushTasksQuery[queueName]) return _flushTasksQuery[queueName]
    ...
    _flushTasksQuery[queueName].SELECT.where.map(w => _inlineLiterals(w))
    _flushTasksQuery[queueName].SELECT.columns = Object.keys(entity.elements).map(k => ({ ref: [k] }))
    _flushTasksQuery[queueName].forUpdate({ ignoreLocked: true })
  }
...
const _processTasks = (queueName, tenant, _opts = {}) => {
  ...
  return jobExecutor.run({ target: queueName, tenant }, () => {
    ...
    const _begin = cds.spawn.bind(cds)
    ...
    const processing_fn = async () => {
      ...
      const tasksGen = function* () {
        for (const _task of selectedTasks) {
          ...
          // older messages have no `service`; the name is encoded in `target` as `<appid><serviceName>`
          let srvName = _msg.service
          if (!srvName) {
            srvName = _task.target
            const appid = cds.env.appid
            if (appid && srvName.startsWith(appid)) srvName = srvName.slice(appid.length)
          }
          delete _msg.service
          ...
          try {
            ...
            const result = await _run({ ...task.context, tenant }, async () => {
              ...
              if (is_t0) {
                ...
                const _delete = DELETE.from(OUTBOX).where({ ID: task.ID })
                // guard on the selected row's timestamp so a concurrent unschedule/reschedule is
                // serialized against the recurrence recreated below
                if (task.msg.queue?.task)
                  _delete.where({ timestamp: { '<': _timestampPlusOneMS(new Date(task._task.timestamp)) } })
                ...
                // #succeeded callback — always owed once the in-flight execution completed
                ...
                // next occurrence — only if the guarded delete removed the row (else it was unscheduled/
                // rescheduled concurrently); non-singletons always recreate
      ...
      if (toBeUpdated.length) {
        await _tx(async () => {
          ...
          for (const each of toBeUpdated) {
            ...
            each.updateData.lastAttemptTimestamp = cds.context?.timestamp ?? new Date().toISOString()
