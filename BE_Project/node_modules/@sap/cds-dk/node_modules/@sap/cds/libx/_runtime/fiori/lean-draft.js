...
const compileUpdatedDraftMessages = (newMessages, persistedMessages, requestData, draftsRef) => {
  ...
  const draftMessageTargetPrefix = cds.odata.urlify(
    { query: { SELECT: { from: { ref: prefixRef } } }, subject: { ref: prefixRef } },
  ...
  const newMessagesByMessageKeyAndTarget = newMessages.reduce((acc, message) => {
    ...
    message.prefix = cds.odata.urlify(
      { query: { SELECT: { from: { ref: messagePrefixRef } } }, subject: { ref: messagePrefixRef } },
      { kind: 'odata-v4', model: txModel }
    ).path
...
const draftHandle = async function (req) {
  ...
  if (
    ...
    if (!newDraftAction) {
      ...
      if (Array.isArray(result)) result.forEach(d => (d.IsActiveEntity = true))
      else result.IsActiveEntity = true
  ...
  if (shouldHandleNewDraftAction && req.event === newDraftAction) {
    ...
    _deepRemoveIsActiveEntity(innerReq.data, innerReq.target)
  ...
  if (
    ...
    if (req.event === 'NEW') _deepRemoveIsActiveEntity(_req.data, _req.target)
  ...
  if (req.event === 'PATCH' || (req.event === 'UPDATE' && req.target.drafts)) {
    ...
    const _rmIsActiveEntity = (data, target) => {
      ...
      for (const assoc in target.associations) {
        ...
        if (val && typeof val === 'object') {
          const _target = target.associations[assoc]._target
    ...
    if (draftParams.IsActiveEntity === false) {
      ...
      // Propagate parent's DraftUUID into nested composition children
      const draftUUID = res.DraftAdministrativeData_DraftUUID
      for (const key in req.data) {
        const elem = req.target.elements[key]
        if (!elem?.isComposition) continue
        const val = req.data[key]
        if (Array.isArray(val)) req.data[key] = val.map(v => _createNewDraftData(v, elem._target, draftUUID))
        else if (val && typeof val === 'object') req.data[key] = _createNewDraftData(val, elem._target, draftUUID)
      }
      ...
      if (IS_PERSISTED_DRAFT_MESSAGES_ENABLED && row.DraftMessages?.length) {
        ...
        const filteredDraftMessages = row.DraftMessages.reduce((acc, msg) => {
          ...
          const queryPrefixUrl = cds.odata.urlify(
            { query: { SELECT: { from: { ref: queryPrefixRef } } }, subject: { ref: queryPrefixRef } },
            { kind: 'odata-v4', model: txModel }
          ).path
          const msgPrefixUrl = cds.odata.urlify(
            { query: { SELECT: { from: { ref: msgPrefixRef } } }, subject: { ref: msgPrefixRef } },
            { kind: 'odata-v4', model: txModel }
          ).path
...
function _deepRemoveIsActiveEntity(data, target) {
  if (!data) return
  ...
  for (const key in data) {
    if (!target.elements[key]) return
    if (data[key] && target.elements[key].isAssociation) delete data[key].IsActiveEntity
    if (!target.elements[key].isComposition) continue
    if (Array.isArray(data[key])) data[key].map(v => _deepRemoveIsActiveEntity(v, target.elements[key]._target))
    else if (typeof data[key] === 'object') _deepRemoveIsActiveEntity(data[key], target.elements[key]._target)
  }
}
...
module.exports = cds.service.impl(function () {
  ...
  this.edit = function (active, key) { return this.send('EDIT', active, key) }
  this.save = function (draft, key) { return this.send('SAVE', draft, key) }
  this.cancel = function (draft, key) { return this.send('CANCEL', draft, key) }
  this.discard = function (draft, key) { return this.send('DISCARD', draft, key) }
  ...
  // NOTE: These handler registrations MUST use inline functions
  // NOTE: > Arrow functions or functions bound with `.bind` will 
  // NOTE: > ... not be able to access the context tx from `this` 
  this.on('NEW', '*', function (req, next) { return onNew.call(this, req, next) })
  this.on('EDIT', '*', function (req, next) { return onEdit.call(this, req, next) })
  this.on('CANCEL', '*', function (req, next) { return onCancel.call(this, req, next) })
  this.on('draftPrepare', '*', function (req, next) { return onPrepare.call(this, req, next) })
})
