...
class Config {
  ...
  constructor (_context, _home, _defaults=true) {
    ...
    this._normalize_queue_config()
    if (this.requires?.scheduling) {
      if (!this.requires.db) this.requires.scheduling = false
      if (!this.requires.queue) this.requires.scheduling = false
    }
    ...
    if (this.features?.emulate_vcap_services) this._emulate_vcap_services()
  }
  ...
  _normalize_queue_config() {
    if (!this.requires?.queue && !this.requires?.outbox) return
    if (this.requires.queue && typeof this.requires.queue !== 'object') this.requires.queue = {}
    if (this.requires.outbox) { // REVISIT: remove with cds^11
      // can't use cds.utils.deprecated here, because it uses cds.env, which results in a endless loop
      console.warn(
        '\x1b[33m',
        '\n------------------------------------------------------------------------------',
        '\nDEPRECATED: cds.requires.outbox\n',
        '\n   Configuration cds.requires.outbox is deprecated and will be removed in upcoming releases!',
        '\n   => Please use cds.requires.queue instead.\n',
        '\n------------------------------------------------------------------------------\n',
        '\x1b[0m')
      this.requires.queue ??= {} // Handle scenarios with _defaults=false
      if (typeof this.requires.outbox === 'object') Object.assign(this.requires.queue, this.requires.outbox)
      if (this.requires.queue.kind === 'persistent-outbox') this.requires.queue.kind = 'persistent-queue'
    }
  }
...
function _merge (dst, src, _profiles) {
  ...
  for (let p in descr) {
    ...
    if (typeof v === 'object' && !Array.isArray(v) && v != null) {
      if (!dst[p]) dst[p] = {}
      if (typeof dst[p] !== 'object' || Array.isArray(dst[p])) dst[p] = v
      else _merge (dst[p], v, _profiles)
      continue
    }
    else if ((v === true || typeof v === 'string') && typeof dst[p] === 'object' && dst[p]?.kind) {
      if (typeof v === 'string') dst[p].kind = v  // requires.db = 'foo' -> requires.db.kind = 'foo'
    }
    else if (v !== undefined) dst[p] = v
  }
  if (profiled.length > 1 && !_profiles.has('production')) {
...
function _readYaml (file) {
  if (isfile(file)) {
    const YAML = _readYaml.parser ??= require('yaml')
