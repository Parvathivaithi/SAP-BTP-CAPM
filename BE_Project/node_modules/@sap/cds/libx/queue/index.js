/*
 * old non-scheduling-based event queue processing
 */
...
cds.on('shutdown', () => (taskRunner._shutdown = true))

const { INTERNAL_USER, PROCESSING, QUEUE } = require('./consts')
const { targetName, create_task } = require('./utils')
...
const _timestampPlusOneMS = timestamp => new Date(timestamp.getTime() + 1).toISOString()
...
const _processTasks = (target, tenant, _opts = {}) => {
  ...
  return taskRunner.run({ name: target, tenant }, () => {
    ...
    const spawn = _begin(async () => {
      ...
      // Get tasks to process from the database

      const whereClause = []
      if (cds.env.appid && !opts.targetPrefix) {
        whereClause.push(...cds.ql.where({ appid: cds.env.appid, target: target }).where)
      } else if (cds.env.appid && opts.targetPrefix) {
        whereClause.push(...cds.ql.where({ appid: cds.env.appid, target: target }).where)
        whereClause.push('or')
        whereClause.push(...cds.ql.where(`appid is null and startswith(target, '${opts.targetPrefix}') = true`).where)
      } else {
        whereClause.push(...cds.ql.where({ target: targetName(target, opts) }).where)
      }

      // Legacy Fallback: To flush messages which have been stored with target = service.name
      if (target === QUEUE && opts[$service]) {
        whereClause.push('or')
        whereClause.push(...cds.ql.where({ appid: null, target: targetName(opts[$service], opts) }).where)
      }
      ...
      if (opts.parallel) {
        tasksQuery.forUpdate({ ignoreLocked: true })
      ...
      try {
        ...
        // REVISIT: server is shutting down, no need to reschedule, just exit
        if (taskRunner._shutdown) return _done()

        // could potentially be a timeout
        const attempt = opts.attempt + 1
        const _waitingTime = waitingTime(attempt)
        // prettier-ignore
        LOG.error(`${target}: Message retrieval failed`, e, `Retry attempt ${attempt} in ${Math.round(_waitingTime / 1000)} s`)
        taskRunner.plan({ name: target, tenant, waitingTime: _waitingTime }, () =>
          _processTasks(target, tenant, { ...opts, attempt })
        )
        return _done()
      }
      ...
      const tasksGen = function* () {
        for (const _task of selectedTasks) {
          ...
          // older messages have no `service`; the name is encoded in `target` as
          // `<targetPrefix><serviceName>` (see utils.targetName)
          let srvName = _msg.service
          if (!srvName) {
            srvName = _task.target
            if (opts.targetPrefix && srvName.startsWith(opts.targetPrefix))
              srvName = srvName.slice(opts.targetPrefix.length)
          }
          delete _msg.service
      ...
      try {
        const _handleWithErr = async task => {
          if (!cds.services[task.srvName]) {
            // service not served/registered: unrecoverable, so dead-letter it (attempts -> maxAttempts)
            // instead of crashing the app or blocking the queue
            LOG.error(`No service '${task.srvName}' found for a queued message; treating as unrecoverable.`)
            if (!minAttemptFailed) minAttemptFailed = task.attempts
            else minAttemptFailed = Math.min(minAttemptFailed, task.attempts)
            task[$error] = new Error(`No service '${task.srvName}' found`)
            if (opts.maxAttempts) {
          ...
          try {
            ...
            const result = await _run({ ...task.context, tenant }, async () => {
              ...
              if (!opts.legacyLocking) {
                const _delete = DELETE.from(tasksEntity).where({ ID: task.ID })
                // guard on the selected row's timestamp (not `task.msg.timestamp`: `timestamp` is in
                // `_ignoredContext`, absent from the msg, and reads back as `now`)
                if (task.msg.queue?.task)
                  _delete.where({ timestamp: { '<': _timestampPlusOneMS(new Date(task.timestamp)) } })
                const _deleted = await _delete
                const _cnt = typeof _deleted === 'number' ? _deleted : (_deleted?.affected ?? 0)
                // recreate the next occurrence in the same tx; skip if the guarded delete removed nothing
                // (the singleton was unscheduled/rescheduled concurrently)
                if (task.msg.queue?.every && (!task.msg.queue?.task || _cnt > 0)) {
                  const _m = { ...task.msg }
                  _m._fromSend = task.msg instanceof cds.Request
                  await INSERT.into(tasksEntity).entries(create_task(target, _m, task.context, task.srvName, opts, true))
                  // re-trigger the drain to schedule the new occurrence
                  _setWaitingTime(0)
                }
              }
              return result
      ...
      if (_toBeDeleted.length) {
        const where = []
        for (const task of _toBeDeleted) {
          if (where.length) where.push('or')
          where.push({ ref: ['ID'] }, '=', { val: task.ID })
          // only delete singleton task if not modified in the meantime
          if (task.msg.queue?.task)
            where.push('and', { ref: ['timestamp'] }, '<', { val: _timestampPlusOneMS(task.msg.timestamp) })
        }
        queries.push(DELETE.from(tasksEntity).where(where))
      }
      ...
      for (const each of toBeUpdated) {
        ...
        each.updateData.lastAttemptTimestamp = cds.context?.timestamp ?? new Date()
      ...
      const _failed = async task => {
        ...
        if (service.handlers.on.some(h => h.for(msg)) || service.handlers.after.some(h => h.for(msg))) {
          toBeCreated.push(create_task(target, msg, task.context, task.srvName, opts))
        }
      }
      ...
      for (const task of succeeded) {
        ...
        // handle `every` — !legacyLocking recreates in the success tx above; the legacyLocking path
        // (deferred delete) recreates here, using a read-back to detect a concurrent unschedule/reschedule
        if (task.msg.queue?.every && opts.legacyLocking) {
          const _m = { ...task.msg }
          if (task.msg.queue.task) {
            const dbTask = await SELECT.one(tasksEntity).where({ ID: task.ID })
            if (!dbTask) continue //> was unscheduled -> no next occurrence
            Object.assign(_m, JSON.parse(dbTask.msg))
          }
          _m._fromSend = task.msg instanceof cds.Request
          const _task = create_task(target, _m, task.context, task.srvName, opts, true)
          toBeCreated.push(_task)
        }
      }
      // invoke failed handlers (only if max attempts is reached)
      for (const task of toBeUpdated) {
        // skip dead-lettered messages whose service is not served/registered (see above)
        if (!cds.services[task.srvName]) continue
        if (
...
const _writeInQueue = async (name, msg, context, serviceName, taskOpts) => {
  const taskMsg = create_task(name, msg, context, serviceName, taskOpts)
  const tasksEntity = _getTasksEntity()
  LOG._debug && LOG.debug(`${name}: Write message to queue`)
  if (msg.queue?.task) return UPSERT.into(tasksEntity).entries(taskMsg)
...
exports.queued = function queued(srv, opts) {
  ...
  if (queueOpts.targetPrefix)
    cds.utils.deprecated({ kind: 'Configuration', old: 'cds.requires.queue.targetPrefix', use: 'cds.appid' })
  ...
  queuedSrv._unschedule = async function (task) {
    const query = DELETE.from(_getTasksEntity()).where({ task })
    const target = targetName(srv.name, queueOpts)
    if (cds.env.appid) {
      if (queueOpts.targetPrefix)
        query.where`appid = ${cds.env.appid} OR appid IS NULL AND startswith(target, '${queueOpts.targetPrefix}') = true`
      else query.where({ appid: cds.env.appid })
    }
    ...
    query.where`target = ${target} OR msg LIKE ${`%"service":"${srv.name}"%`}`
    return await query
  }
